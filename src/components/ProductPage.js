      ...
      <div className="bg-[#fffbef]" ref={productHeroRef}>
        ...
        <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 py-6 px-4">
              ...
              <div className="hidden md:block">
                ...
                {productImages.length > 1 && (
                  <div className="relative">
                        ...
                        <button 
                          ...
                          className={`relative flex-shrink-0 w-20 aspect-square rounded-lg overflow-hidden transition-all duration-300 ${
                            selectedImage === image.id 
                              ...
                              : 'border border-transparent hover:border-[#FF6B6B]'
                          }`}
                        >
                          <img 
                            src={image.src}
                    ...
                    {/* Progress indicator - removing since it's not in the reference design */}
          ...
          <section aria-labelledby="product-details-heading">
            ...
            {/* Title */}
            <h1 className="text-2xl font-medium mb-1 md:mb-1">
              {(() => {
            ...
            {/* Product Description */}
            <p className="text-gray-600 text-base md:text-lg mb-6 whitespace-normal">
              {(() => {
                const firstSentence = product.fullDescription.split('.')[0];
                // If the sentence is too long, try to find a natural break point
                if (firstSentence.length > 60) {
                  const words = firstSentence.split(' ');
                  const shortenedDesc = words.slice(0, 8).join(' ');
